...
### 🔧 前后端API连接修复 ⭐ 修复 (v2.3.6)
- **前端API配置修复**：修复前端axios baseURL配置，解决重复/api路径问题
- **API调用路径优化**：统一前端API调用路径，消除URL重复拼接错误
- **后端健康检查**：添加缺失的/api/health端点，确保前端健康检查正常
- **SQLite日期处理修复**：修复evaluation_history_service.py中日期格式兼容性问题
- **生产环境部署验证**：确保前后端在生产环境9.135.87.101正常连接
- **全面诊断工具**：新增前后端连接诊断脚本，快速定位连接问题

### 🔧 SQLAlchemy兼容性修复 ⭐ 修复 (v2.3.5保持)
- **SQLAlchemy 2.0+兼容**：修复`engine.execute()`方法已废弃的API兼容性问题
- **向后兼容保障**：支持新旧版本SQLAlchemy，确保在不同环境中稳定运行
- **数据库连接优化**：使用现代化的`with db.engine.connect()`连接管理模式
- **生产环境修复**：解决线下环境后端启动时的数据库连接错误
- **全面测试验证**：新增数据库连接测试脚本，确保修复有效性

### 🎯 生产环境初始化修复 ⭐ 修复 (v2.3.5)
- **正确服务导入**：修复生产环境初始化脚本中ClassificationService导入错误
- **完整服务支持**：使用SQLite版本ClassificationService，确保所有方法可用
- **初始化验证**：生产环境数据库初始化脚本完全正常运行
- **健康检查完善**：增强生产环境数据库健康状态检查功能

### 🌐 生产环境部署优化 ⭐ 修复 (v2.3.4 保持)
- **前端启动增强**：解决生产环境npx serve不可用问题，新增4层级HTTP服务器备选方案
- **数据库连接修复**：增强生产环境数据库路径验证、权限检查和临时目录备选机制
- **专用初始化脚本**：新增`init_production_db.py`生产环境数据库初始化工具
- **智能数据迁移**：新增`migrate_production_data.py`自动查找和迁移历史数据
- **权限自动设置**：生产环境数据库目录和文件权限自动配置
- **完整诊断工具**：详细的错误诊断和环境健康检查
...
### v2.3.6 (2025-12-05) - 前后端API连接修复版本
- 🔧 **前后端连接修复**：彻底解决生产环境前后端API连接问题
- ✨ 前端API配置优化：修复axios baseURL重复/api路径拼接问题
- ✨ 统一API调用路径：前端组件统一使用正确的API路径格式
- ✨ 后端健康检查：新增/api/health端点，确保前端健康检查正常
- ✨ SQLite日期兼容：修复evaluation_history_service.py日期格式处理
- ✨ 全面诊断工具：新增`diagnose_frontend_backend.py`连接诊断脚本
- ✨ 测试API工具：新增`test_frontend_api_call.py`模拟前端API调用
- 📋 修复文档：详细记录修复过程和解决方案
- 🚀 生产验证：确保9.135.87.101:7860/8701环境正常运行
