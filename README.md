...
智能问答评估系统，基于大语言模型的自动化评估平台，支持多维度评分、智能分类识别、历史记录管理、统计分析、人工评估修正、配置数据全量同步、多环境部署和完整的图片识别功能。

## 🚀 最新功能特性 (v4.0.0) ⭐ 重大版本

### 🤖 AI助手功能完善 ⭐ 核心修复
- **修复前端错误**：彻底解决 `handleAskAI` 函数未定义的eslint错误
- **完整AI助手实现**：新增完整的AI助手功能，支持基于用户问题智能生成回答
- **智能问答工作流**：用户输入问题 → 点击"问AI" → AI生成回答 → 自动填入"模型回答"框
- **上下文理解**：AI助手能够结合评估标准和参考答案提供更准确的回答
- **错误处理优化**：完善的错误处理机制和用户反馈提示
- **后端服务支持**：新增 `backend/services/ai_assistant.py` 提供AI助手API服务

### 📊 维度统计页面简化 ⭐ 用户体验优化
- **修正数据计算**：解决"总评估次数"重复计算问题（AI+人工记录去重）
- **简化统计指标**：移除"分类数量"和"维度总数"，聚焦核心指标
- **人工评分优先**：平均表现统计改为仅基于人工评分，提高数据准确性
- **界面简化**：移除冗余的"各维度整体表现"和"分类表现对比"数据块
- **重点突出**：总览页面更加简洁明了，突出最重要的统计信息

### 🎯 UI/UX界面优化 ⭐ 简化设计
- **按钮逻辑优化**：删除多余的"问AI"按钮，保留唯一合理的AI助手入口
- **工作流清晰**：用户输入问题 → 问AI → 生成模型回答 → 手动输入参考答案
- **交互一致性**：消除界面中的混乱和逻辑冲突，提升用户体验
- **视觉优化**：统计卡片布局优化，从4个精简为2个核心指标

### 🛠️ 数据管理工具 ⭐ 新增功能
- **数据清理脚本**：新增 `backend/cleanup_dapan_data.py` 支持批量数据清理
- **智能备份机制**：完善的数据备份和恢复流程
- **数据导入优化**：更新大盘行业分析数据导入功能

### 🔧 技术架构改进
- **代码质量提升**：修复eslint错误，提高代码可维护性
- **前端组件优化**：简化组件结构，减少代码冗余
- **数据流优化**：统一数据计算逻辑，避免重复计算
- **API接口完善**：新增AI助手相关API接口

### 📈 版本发布流程
- **完整版本控制**：Git标签管理，完整的版本发布流程
- **数据备份保障**：版本发布前自动创建数据备份
- **文档同步更新**：项目文档与功能同步更新

## 🚀 v3.0.0 历史功能特性

### 📸 智能图片识别与管理系统 ⭐ 全新功能
- **完整图片上传工作流**：支持文件上传和Ctrl+V粘贴上传
- **OCR智能识别**：基于Tesseract.js的中英文文字识别
- **服务器持久化存储**：图片保存到服务器资源目录，永久可访问
- **图片历史管理**：评估历史中完整展示所有使用的图片
- **智能URL管理**：使用服务器URL替代临时blob链接
- **预览与管理**：支持图片预览、OCR状态显示、大图查看
- **数据库集成**：图片信息完整保存在评估历史中

### 🔧 图片上传技术架构
- **双重上传方式**：FormData文件上传 + Base64粘贴上传
- **安全验证**：文件格式、大小限制、图片完整性验证
- **唯一文件名**：时间戳+UUID确保文件名唯一性
- **静态资源服务**：`/api/static/uploads/images/`图片访问接口
- **网络断点续传**：支持大文件上传的错误处理和重试

### 📱 用户交互体验
- **拖拽上传**：支持直接拖拽图片到指定区域
- **粘贴上传**：Ctrl+V快速粘贴截图或复制的图片
- **实时预览**：上传后立即显示图片预览
- **OCR进度提示**：文字识别过程的实时进度显示
- **自动文本填充**：识别的文字自动填入评估表单
- **图片管理界面**：缩略图展示、状态标识、删除管理

### 🗄️ 数据库结构升级
- **新增字段**：`evaluation_history.uploaded_images_json`存储图片信息
- **完整元数据**：包含文件名、大小、类型、URL、OCR结果
- **历史记录增强**：评估历史页面完整展示历史图片
- **向后兼容**：现有数据不受影响，平滑升级

### 🌐 API接口扩展
- **图片上传接口**：`POST /api/upload/image`
- **静态文件服务**：`GET /api/static/uploads/images/<filename>`
- **清理接口**：`POST /api/upload/images/cleanup`
- **错误处理**：完整的错误响应和状态码

### 🎯 解决的核心问题
- ❌ **图片信息丢失**：历史记录中看不到评估时使用的图片
- ❌ **临时URL失效**：页面刷新后图片链接失效
- ❌ **OCR结果无法保存**：识别的文字无法与评估记录关联
- ✅ **完整图片工作流**：从上传到历史记录的完整链路
- ✅ **持久化存储**：服务器存储确保图片永久可访问
- ✅ **智能识别集成**：OCR与评估系统完美结合
...
| 版本 | 发布日期 | 主要功能 | 核心改进 |
|------|----------|----------|----------|
| **v4.0.0** | 2025-06-10 | **AI助手完善 + UI简化** | 🤖 修复前端错误，完善AI助手功能<br/>📊 简化统计页面，优化数据计算<br/>🎯 界面简化，提升用户体验 |
| v3.0.0 | 2025-06-XX | **图片识别系统** | 📸 完整图片上传工作流<br/>🔧 OCR智能识别<br/>🗄️ 数据库结构升级 |
| v2.3.6 | 2025-12-05 | **API连接修复** | 🔧 前后端API连接修复<br/>✨ SQLite兼容性优化<br/>🚀 生产环境验证 |
| v2.3.2 | 2025-06-03 | **多环境配置** | 🌍 环境智能切换<br/>🔒 重复记录防护<br/>🚀 一键部署脚本 |
| v2.3.1 | 2025-06-03 | **配置同步** | 🔄 全量配置同步<br/>👥 团队协作支持<br/>🛡️ 数据安全保障 |
| v2.0.0 | 2025-01-XX | **人工评估系统** | 👨‍💼 AI+人工双重评估<br/>📊 评估修正功能<br/>🎨 对比展示界面 |

### v4.0.0 突出亮点 ⭐

| 功能模块 | v3.0.0 状态 | v4.0.0 改进 | 用户价值 |
|----------|-------------|-------------|----------|
| **AI助手** | ❌ 前端错误无法使用 | ✅ 完整功能，智能问答 | 🚀 用户可正常使用AI生成回答 |
| **统计页面** | ⚠️ 数据重复计算混乱 | ✅ 数据准确，界面简洁 | 📊 统计数据更准确，页面更清晰 |
| **用户界面** | ⚠️ 多个问AI按钮困惑 | ✅ 逻辑清晰，操作简单 | 🎯 用户操作更直观，体验更好 |
| **数据计算** | ❌ AI+人工记录重复计算 | ✅ 准确计算，去重处理 | 📈 数据统计更可靠 | 