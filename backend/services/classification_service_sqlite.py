...
请严格按照以下格式返回评估结果:
总分: [分数]/10
评分理由: [详细的多行评分分析，必须说明扣分理由，特别关注选股建议的专业性缺陷和风险提示不足]"""
            },
            '分析': {
...
评分指导原则：
- 8-10分：仅给予分析深度极强、逻辑严密、数据准确的专业回答
- 5-7分：分析基本到位但存在不足的回答
- 2-4分：分析浅显或存在明显问题的回答
- 0-1分：分析错误或严重缺乏逻辑的回答
...
请严格按照以下格式返回评估结果:
总分: [分数]/10
评分理由: [详细的多行评分分析，必须指出分析的具体不足和专业性缺陷]""",
...
评分指导原则：
- 8-10分：仅给予分析专业、数据准确、逻辑清晰的高质量回答
- 5-7分：基本分析到位但深度或准确性不足的回答
- 2-4分：分析简单或存在错误的回答
- 0-1分：分析严重错误或完全不专业的回答

请严格按照以下格式返回评估结果:
总分: [分数]/10
评分理由: [详细的多行评分分析，必须说明个股分析的具体问题和专业性不足之处]"""
            },
            '决策': {
...
评分指导原则：
- 8-10分：仅给予分析透彻、建议具体、风险评估充分的专业回答
- 4-7分：基本合理但存在明显不足的回答
- 1-3分：建议缺乏依据或风险评估不足的回答
- 0分：误导性或完全不负责任的回答

请严格按照以下格式返回评估结果:
总分: [分数]/10
评分理由: [详细的多行评分分析，必须说明投资建议的问题和风险评估的不足]"""
            },
            '信息查询': {
...
评分指导原则：
- 8-10分：仅给予信息完全准确、高度相关、表达简洁的回答
- 5-7分：信息基本准确但存在不足的回答
- 2-4分：信息部分错误或相关性不强的回答
- 0-1分：信息错误或完全不相关的回答

请严格按照以下格式返回评估结果:
总分: [分数]/10
评分理由: [详细的多行评分分析，必须说明信息准确性和实用性的具体问题]"""
            }
        }
...
请严格按照以下格式返回评估结果:
总分: [分数]/10
评分理由: [详细的多行评分分析，必须说明扣分理由，按照评估标准逐项说明问题，特别注明时间因素的考虑]"""
